...
def generate_partner_pricing_data(n_partners=900, seed=42):
    ...
    bookings_after = bookings_baseline * (1 + treatment * true_uplift - treatment * cannibalization_pressure + noise)
    ...
    cancellation_rate_before = np.clip(0.18 - quality_score / 900 + rng.normal(0, 0.025, n_partners), 0.03, 0.36)
    cancellation_rate_after = np.clip(
        cancellation_rate_before + np.where(treatment == 1, rng.normal(0.005, 0.015, n_partners), rng.normal(0.0, 0.012, n_partners)),
        0.02,
        0.40
    )
...
if __name__ == "__main__":
    out = Path(__file__).resolve().parents[1] / "data" / "synthetic" / "b2b_pricing_experiment_synthetic_data.csv"
